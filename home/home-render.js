...
function renderCards(list) {
    ...
    list.forEach((card, i) => {
        container.insertAdjacentHTML('beforeend', ` <article key="${card.id}">
    ...
    list.forEach(card => {
        let key = document.querySelector(`article[key="${card.id}"]`)
...
function renderSingleCard(list) {
    ...
    list.forEach((card, i) => {
        container.insertAdjacentHTML('beforeend', `<article class="single-card" key="${card.id}" onclick="renderModal(${card.id})">
    ...
    const modal = `
        <div class="modal" style="background-image:  linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, #F0F0F0 63.23%, #F0F0F0 75.85%), url('${game.background_image}');">
            ...
            <div class="modal-text">
                ...
                <div class="heart-icon-modal"><img src="./assets/modal/Heart.svg"></div>
                    </div>
                ...
                <div class="left-column">
                        ...
                        <p class="platforms-modal-title">Platforms</p>
                            <p class="platforms-modal-sp">${getPlatforms(game.platforms)}</p>
                            </div>
                            ...
                            <p class="platforms-modal-sp">${getName(game.publishers)}</p>
                            </div>
                        <div class="website-modal">
                            ...
                            <p class="platforms-modal-sp">${getGenre(game.genres)}</p>
                        </div>
                        <div class="developer-modal">
                            <p class="platforms-modal-title">Developer</p>
                            <p class="platforms-modal-sp">${getName(game?.developers|| "Not defined")}</p>
                            </div>
                            <div class="age-modal">
...
let searchBar = document.querySelector(".search-games");
...
function renderList(list) {
    ...
    list.forEach(element => {
     let filteredList = document.querySelector(`.games-list-game[data-id="${element.id}"]`)
     let completedList = document.querySelector('.list')
